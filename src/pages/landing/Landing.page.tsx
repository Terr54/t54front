...
const Landing = () => {
  ...
  const options = [{ id: 0, title: 'Buyers' }, { id: 1, title: 'Suppliers' }];
  ...
  return (
    <Container>
      ...
      <small>Photo by <a href="https://unsplash.com/@markusspiske?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Markus Spiske</a> on <a href="https://unsplash.com/s/photos/agriculture?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
  </small>
      {/* <ResponsiveDrawer /> */}
      ...
      <SectionWrapper height={100} marginTop='20px'>
        <HeaderCaption
      ...
      <RButtons>
        ...
        <MerchantActions>
            <HeaderCaption heading={t('extend-reach')} caption={t('extend-reach-text')} color="#000" textAlign='left' captionLength="80%" alignItems='flex-start' />
            <Right>
            <Actions>
              { activeTab === t('buyers')
                ? merchantActions.filter((it, idx) => idx > 1).map((action, index) => (
                  <ServiceCard key={action.caption} bgColor='#F9F9F9' width="80vw" margin="8px" padding="15px">
                    <HeaderCaption heading={action.title} caption={action.caption} color="#531703" textAlign='left' headingMargin='0px' headingSize='8px'>
                      <ActionCount>
                        <span>{`0${index + 1}`}</span>
                      </ActionCount>
                    </HeaderCaption>
                ...
                : (
                    merchantActions.filter((it, idx) => idx <= 1).map((action, index) => (
                    <ServiceCard key={action.caption} bgColor='#F9F9F9' width="80vw" margin="8px" padding="15px 15px">
                      <HeaderCaption heading={action.title} caption={action.caption} color="#531703" textAlign='left' headingMargin='0px' headingSize='8px'>
                        <ActionCount>
                          <span>{`0${index + 1}`}</span>
                        </ActionCount>
      ...
      <SectionWrapper height={100} padding="0px 44px 44px">
        <HeaderCaption
          ...
          headingDisplay='flex'
        >
           <Button bgColor='transparent' color='#E63F07' text='See All' boxShadow='none' onClick={() => navigate(path.HERBSANDSPICES)} width='8%' />
        </HeaderCaption>
        <ServicesContainer
          style={{
            display: 'flex',
      ...
      <SectionWrapper height={100}>
        ...
        <ServicesContainer>
          {
            blogData.filter((d, idx) => idx < 3).map((data, idx) => (
              <BlogCard key={data.id} date={data.date} tradeType={data.tradeType} caption={data.caption} heading={data.heading} bgImage={data.bgImage} margin={idx === 1 ? '0px 16px' : ''} onClick={() => navigate(`/blog/cultivation/${data.id}`)} />
            ))
          }
        </ServicesContainer>
        ...
        <Button
          ...
          padding="0px"
          fontSize="13px"
          renderIcon={() => <ArrowForward style={{ width: '13px', height: '13px' }} />}
